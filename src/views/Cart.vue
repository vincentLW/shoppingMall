<template>
    <div>
      ...
      <Modal v-bind:mdShow="modalConfirm" v-on:close="closeModal()">
        ...
        <p slot="message">Are you sure to delete this item</p>
        <div slot="btnGroup">
          <a class="btn btn--m" @click="delCart()">Confirm</a>
          <a class="btn btn--m" @click="modalConfirm=false">Close</a>
        </div>
      </Modal>
    ...
    export default {
        ...
        methods:{
          ...
          editCart(flag,item){
            ...
            axios.post('/users/cartEdit',{productId:item.productId,productNum:item.productNum})
              .then((response)=>{
                let res=response.data;
